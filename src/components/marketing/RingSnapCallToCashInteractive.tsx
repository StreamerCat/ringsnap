...
const STEPS = [
    { id: 1 as Step, title: "Answer fast", description: "Under 2 rings, 24/7", icon: Phone },
    { id: 2 as Step, title: "Qualify", description: "Job type, urgency, location, budget fit", icon: ClipboardList },
    { id: 3 as Step, title: "Handle objections", description: "Pricing, timing, comparisons, trust questions", icon: MessageSquare },
    { id: 4 as Step, title: "Book or hand off", description: "Appointment booked or clean transfer with notes", icon: Calendar },
    { id: 5 as Step, title: "Improve automatically", description: "Uses outcomes and patterns to refine talk tracks and routing", icon: TrendingUp },
];

const SCENARIOS: Record<Scenario, { label: string; icon: React.ElementType }> = {
    "emergency": { label: "Emergency", icon: AlertTriangle },
    "price-shopper": { label: "Price Shopper", icon: DollarSign },
    "repeat-customer": { label: "Repeat Customer", icon: UserCheck },
};
...
const PROOF_CHIPS = [
    "Booked jobs, not voicemails",
    "Fewer missed calls",
    "Better close rate from the same leads",
    "Consistent talk track across every call",
];
                ...
                <TabsList className="grid w-full grid-cols-3 h-auto p-1 bg-muted/50 rounded-xl mb-6">
                    {Object.entries(SCENARIOS).map(([key, { label, icon: Icon }]) => (
                        <TabsTrigger
                            ...
                            value={key}
                            className={cn(
                                "flex items-center gap-2 py-3 px-4 rounded-lg text-sm font-medium transition-all",
                                    ...
                                    <button
                                        ...
                                        className={cn(
                                            ...
                                            isActive
                                                ...
                                                : "border-muted bg-white hover:border-primary/30 hover:shadow-sm"
                                        )}
                                    >
                                        <div
                                            className={cn(
            ...
            <div className="flex flex-wrap justify-center gap-3 mb-4">
                {PROOF_CHIPS.map((chip) => (
                    <Badge
                        key={chip}
                        variant="outline"
                        className="px-4 py-2 text-sm font-medium bg-white border-primary/20"
                    >
                        {chip}
                    </Badge>
                ))}
