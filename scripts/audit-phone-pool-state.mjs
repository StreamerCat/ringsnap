 ...
 * Run with: SUPABASE_URL=... SUPABASE_SERVICE_ROLE_KEY=... node scripts/audit-phone-pool-state.mjs
 */
...
const { data: allNumbers, error: statusError } = await supabase
  .from("phone_numbers")
...
if (statusError) {
  ...
  const counts = allNumbers.reduce((acc, row) => {
  ...
  console.log(`  ${"TOTAL".padEnd(15)} : ${allNumbers.length}\n`);
}
...
if (cooldownError) {
  ...
  for (const num of cooldownNumbers) {
    ...
    const cooldownUntil = num.cooldown_until ? new Date(num.cooldown_until).toISOString().slice(0, 10) : "NULL";
    const releasedAt = num.released_at ? new Date(num.released_at).toISOString().slice(0, 10) : "NULL";
    console.log(`    ${phone.padEnd(15)} cooldown_until=${cooldownUntil} released_at=${releasedAt}`);
  }
  console.log();
}
...
if (poolError) {
  ...
  for (const num of poolNumbers) {
    const phone = num.e164_number || num.phone_number;
    const cooldownUntil = num.cooldown_until ? new Date(num.cooldown_until).toISOString().slice(0, 10) : "NULL";
    const lastCallAt = num.last_call_at ? new Date(num.last_call_at).toISOString().slice(0, 10) : "NULL";
    const isReserved = num.is_reserved ? "RESERVED" : "available";
    console.log(`    ${phone.padEnd(15)} last_call=${lastCallAt} is_reserved=${isReserved}`);
  }
  console.log();
}
...
if (releasedError) {
  ...
  for (const num of releasedNumbers) {
    const phone = num.e164_number || num.phone_number;
    const releasedAt = num.released_at ? new Date(num.released_at).toISOString().slice(0, 10) : "NULL";
    console.log(`    ${phone.padEnd(15)} released_at=${releasedAt}`);
  }
  console.log();
}
...
if (TWILIO_ACCOUNT_SID && TWILIO_AUTH_TOKEN) {
  ...
  try {
    ...
    const dbE164Set = new Set(
      dbNumbers?.flatMap(n => [n.phone_number, n.e164_number].filter(Boolean)) || []
    );

    const missingNumbers = twilioNumbers.filter(
      tn => !dbE164Set.has(tn.phone_number)
    );
    ...
    if (missingNumbers.length > 0) {
      ...
      for (const tn of missingNumbers.slice(0, 10)) {
        console.log(`    ${tn.phone_number.padEnd(15)} SID=${tn.sid.slice(0, 20)}...`);
      }
      console.log();
    } else {
    ...
    console.error("Error fetching Twilio inventory:", error.message);
  }
} else {
  console.log("📞 TWILIO INVENTORY: Skipped (credentials not provided)\n");
}
...
const { data: eligibleNumbers, error: eligibleError } = await supabase
  .from("phone_numbers")
  .select("id, phone_number, e164_number, lifecycle_status, cooldown_until, last_call_at, is_reserved")
  .eq("lifecycle_status", "pool");
...
if (eligibleError) {
  ...
  const fullyEligible = eligibleNumbers.filter(n => {
    ...
    const notReserved = !n.is_reserved;
    return cooldownOk && silenceOk && notReserved;
  });

  console.log(`  Total pool numbers: ${eligibleNumbers.length}`);
  console.log(`  Eligible for allocation (after cooldown + 10-day silence + not reserved): ${fullyEligible.length}`);

  if (fullyEligible.length === 0) {
    console.log("\n  ⚠️  NO NUMBERS ELIGIBLE FOR ALLOCATION!");
    console.log("  This explains why the allocator returned NULL.\n");

    // Show why numbers are ineligible
    if (eligibleNumbers.length > 0) {
      console.log("  Reasons for ineligibility:");
      const inCooldown = eligibleNumbers.filter(n => n.cooldown_until && new Date(n.cooldown_until) > now);
      const recentCalls = eligibleNumbers.filter(n => n.last_call_at && new Date(n.last_call_at) >= tenDaysAgo);
      const reserved = eligibleNumbers.filter(n => n.is_reserved);

      if (inCooldown.length > 0) console.log(`    - ${inCooldown.length} in cooldown`);
      if (recentCalls.length > 0) console.log(`    - ${recentCalls.length} received calls within 10 days`);
      if (reserved.length > 0) console.log(`    - ${reserved.length} reserved`);
      console.log();
    }
  } else {
...
// ============================================================================
console.log("🚫 BLACKLISTED NUMBERS STATUS:\n");

const blacklist = ['+19704231415', '+19705168481'];
const { data: blacklistedNumbers, error: blacklistError } = await supabase
  .from("phone_numbers")
  .select("*")
  .or(`phone_number.in.(${blacklist.join(',')}),e164_number.in.(${blacklist.join(',')})`);

if (blacklistError) {
  console.error("Error fetching blacklisted numbers:", blacklistError);
} else if (blacklistedNumbers.length === 0) {
  console.log("  No blacklisted numbers found in database\n");
} else {
  for (const num of blacklistedNumbers) {
    const phone = num.e164_number || num.phone_number;
    console.log(`  ${phone}:`);
    console.log(`    Lifecycle Status: ${num.lifecycle_status || "NULL"}`);
    console.log(`    Assigned Account: ${num.assigned_account_id || "NULL"}`);
